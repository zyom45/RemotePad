...
現時点の Mac Agent はプロトコルと開発体験の検証段階です。`AuthProof` は署名検証前の開発用プレースホルダであるため、本物のペアリングと署名検証が入るまでは次を必須条件にします。
...
- [#2](https://github.com/zyom45/RemotePad/issues/2): 仮認証中は loopback-only とし、Bonjour / LAN 公開を禁止する。
