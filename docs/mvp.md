...
Mac Agentは署名チャレンジ、Mac側承認、失効、Keychain identity、E2E暗号、監査ログを実装済みです。安全側の既定値として次を必須にします。

- 通常起動ではMac Agentはloopback-onlyで待ち受け、Bonjourを公開しない。
- LAN利用時はユーザーが `--lan` を明示する。
- LANモードでも認証後のTerminalとBrowserProxyはE2E暗号化する。
- 外出先接続とRelayは後続マイルストーンとして分離する。
...
1. 安全な既定値: loopback-only、Bonjour無効、LAN公開は明示ゲート。
...
ローカル MVP は次をすべて満たした時点で完成とします。

- 承認済みiPadだけがMac Agentへ認証できる。
- iPad内のANSI対応TerminalからMacのPTYを操作できる。
- iPadの接続が切れてもPTYが継続し、一覧から再接続できる。
- iPad内のWebViewからMac localhostへHTTP、SSE、WebSocketを透過的に中継できる。
- ペアリング、Terminal作成・再接続、BrowserProxyの自動統合検証が通る。
- 実iPadでWebViewのHMR、Cookie、origin、secure-context挙動を確認する。

最後の実機確認以外は自動検証可能です。外出先接続、画面共有、音声はローカル MVP の完成後に続く製品マイルストーンです。
