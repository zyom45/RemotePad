...
Mac AgentはUIペアリング、Mac側承認、失効、監査ログ、E2Eセッションを実装済みです。LAN公開は明示操作に限定します。

- 通常起動は `127.0.0.1` のみにbindする。
- `--lan` 指定時だけLAN bindとBonjour公開を有効にする。
- 認証後は署名済みX25519鍵交換から導出したChaCha20-Poly1305 E2Eセッションを必須にする。
- Relayと外出先接続は未実装であり、LAN公開とは別のリリースゲートを持つ。

外出先へ拡張する条件は、Relayの認証・メタデータ方針・rate limit・独立したセキュリティレビューが揃うことです。
