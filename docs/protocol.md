...
- Transport: TCP
- Auth: デバイス鍵 + 初回ペアリング
- Session: 署名済みX25519鍵交換後、ChaCha20-Poly1305 E2Eチャネル上でRemotePad論理チャネルを開始

実装ゲート:

- Mac Agentは既定で `127.0.0.1` のみへbindし、Bonjourを公開しない。
- `--lan` または `REMOTEPAD_ENABLE_LAN=1` の明示指定時のみLAN待受とBonjourを有効にする。
- LANモードでもTerminalとBrowserProxyは認証済みE2Eフレームだけを受理する。
...
- Media: WebRTC
- E2E: ローカル接続と同じSecureSessionをRelay上で使用する

MVPはTCPを経路として使いますが、機密性と完全性は経路に依存しません。将来のRelayでは、同じ暗号化RemotePadフレームをWebSocketまたはQUIC streamへ載せ替えます。
...
- 鍵交換: 接続ごとのX25519一時鍵
- 鍵導出: HKDF-SHA256、方向別鍵
- 暗号化: ChaCha20-Poly1305
- replay防止: 方向別の単調増加counter

Macは一時鍵を含むServerHello transcriptを長期署名鍵で署名し、iPadはpin済みMac公開鍵で検証します。iPadも両一時鍵を含むAuth transcriptへ署名します。認証成功後の通常フレームはすべて暗号化envelopeへ格納します。
...
現在の実装では、Mac Agentが実際のMac公開鍵を含む `pairing.challenge` を返します。iPad Appは `PairingTranscript` に署名し、Mac鍵をKeychainへpinします。Mac側はSwiftUI承認画面またはCLIで承認・拒否・失効できます。
...
sequenceDiagram
    ...
    I->>M: TCP connect
    I->>M: ClientHello(device_id, nonce, signing_key, ephemeral_x25519)
    M->>I: ServerHello(device_id, nonce, ephemeral_x25519, signature)
    I->>I: pinned Mac key で ServerHello を検証
    I->>M: AuthProof(signature(auth_transcript))
    M->>I: AuthResult(accepted, session_id, permissions)
    I->>M: EncryptedFrame(Terminal / BrowserProxy)
```

セッション開始時に能力交換を行い、利用可能なチャネルと権限を確定します。

クライアントはCurve25519 signing公開鍵と接続ごとのX25519一時公開鍵を送ります。Mac Agentは承認済み署名鍵だけを信頼し、両一時鍵を含む `RemotePad auth v2` transcriptを検証します。
...
```json
{
  ...
  "message": "Client does not support protocol 2.",
  "request_id": 1,
  "supported_protocols": [2],
  "minimum_supported_protocol": 2
}
```
